...
\textbf{LinkedIn: }\url{https://linkedin.com/in/zhang-jiajun} | 
...
\location{\textbf{\color{black}Expected Dec 2017} | GPA: 3.54}
...
\runsubsection{Full Stack Developer} |\descript{\small Interactive Pedestrian Injury Mapper Web App, \color{gray}\url{https://petercanmakit.github.io/IPIM/}} \emph{[Python, Flask, JavaScript, PostgreSQL]}\\
...
\descript{Search Query Expansion} \emph{[Java, Google CSE]}, \descript{\color{headings}Sep 2017 - Oct 2017}\\
\begin{tightitemize}
\item Built an information retrieval system based on Google CSE API
\item Implemented Rocchio algorithm to reformulate the seed query to improve retrieval performance
\item Boosted the average accuracy up to 90\% under two iterations
\end{tightitemize}
\sectionspace 

\descript{UdpChat} \emph{[Java, Socket Programming]}, \descript{\color{headings}Feb 2017 - Mar 2017}\\
\begin{tightitemize}
\item Developed a P2P chatbot with functionalities of online/offline chatting
\item Built the server as it broadcasts the contact information of all users and manages messages for offline users
\item Applied acknowledgment protocol to provide reliable communication
\end{tightitemize}
\sectionspace 

\descript{HTTP Server} \emph{[Linux, C , Socket Programming]}, \descript{\color{headings}Jan 2017 - Feb 2017}\\
\begin{tightitemize}
...
\begin{tightitemize}
\item Built an image processing webpage (\href{https://petercanmakit.github.io/imgProc/}{\color{gray}\url{imgProc}}) \emph{[JavaScript]}
\item Created a music sharing \textbf{Web App} \emph{[Python, SQL, Google Cloud, Flask]}
\item Built a linear \textbf{File System} on loop devices \emph{[Linux Kernel, C]}
\item Implemented a Random-Robin \textbf{Task Scheduler} \emph{[Linux Kernel, C]}
\item Simulated Go-Back-N Transfer Protocol and Distance Vector Routing Algorithm \emph{[Java, Socket Programming]}
\item Created a research tool for motor collision analysis \emph{[Hadoop, Pyspark]}
\end{tightitemize}
\sectionspace 
...
\item Implemented Random Forest algorithm with \textbf{scikit-learn} to enhance the palmprint directional field
